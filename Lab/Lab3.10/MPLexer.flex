...
\<= { return new Symbol( sym.LESS_OR_EQUAL_TO ); }
== { return new Symbol( sym.EQUAL_TO ); }
\<\> { return new Symbol( sym.NOT_EQUAL_TO ); }
=\> { return new Symbol( sym.GREATER_OR_EQUAL_TO ); }
...
//boolean kw
true|false { return new Symbol( sym.CONST, yytext()  ); }
...
//integer
0({octal_array} | (x{hexa_array})) | {digit_array}  { return new Symbol( sym.CONST,  yytext() ); }

//real
(({digit_array}\.{digit_array}?)|(\.{digit_array}))(E[+-]?{digit_array})? { return new Symbol( sym.CONST,  yytext() ); }
